...
- [`metadat`](metadat) Several principal third-party DATs (e.g. No-Intro, Redump, MAME, TOSEC) that each cover a large number of systems and therefore require their own folders in the repository, plus various collections of metadata (some of which may be deprecated).
  - [`arcade`](metadat/arcade) Arcade ClrMamePro XML files to allow [manual scanning of the arcade sets](https://docs.libretro.com/guides/arcade-getting-started/#getting-started-with-arcade-emulation).
